        ...
        <el-col :xs="22" :sm="20" :md="18" :lg="16" :xl="14">
          ...
          <el-row type="flex" justify="space-between">
            ...
            <el-col class="detail-box" :span="4">
              ...
              <div class="text-lg text-bold">5000</div>
            </el-col>
            <el-col class="detail-box" :span="4">
          ...
          <el-row type="flex" justify="center" class="margin-top">
            <el-col class="trading-card">
              <div class="text-and-button">
                <div>
                  <div>本轮投资策略：</div>
                  <div class="text-lg text-bold margin-top-sm">卖出10%</div>
                </div>
...
function calcAllTradingStraregy(cap, buy, sell) {
  ...
  // 最大预期跌幅结果
  const hold_maxFallResult =
    buy + sell * (1 + maxFall);
  // 最大跌幅结果减本金的差
  const hold_reduceFall = hold_maxFallResult - cap;
  const hold_minExpPaybackRate = calcMinExpVol(cap, buy, sell, hold_tradingType, hold_rate);
  const hold_tempRes = {
    title: hold_title,
...
function calcMinExpVol(cap, buy, sell, tradingType, rate) {
  ...
  if (tradingType == "buy") {
    ...
    targetVol = (cap - buy + buy * rate) / (sell + rate) - 1;
  }
  if (tradingType == "sell") {
